...
# REAL-TIME CAMERA (0 for primary webcam)
...
# Start reading frames in an infinite loop
while ret:
    ret, frame = cap.read()
    if ret:
        Ll, A, B = cv2.split(lframe)

        # Check if camera initialized properly
        ret, frame = cap.read()
        cframe = frame
        cv2.imshow("FIRE", frame)

        # OpenCV reads images in BGR format — convert if needed
